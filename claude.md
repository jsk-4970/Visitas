...
> **注意**: 以下は実装済みの構造です。Phase 2/3で追加予定の機能は別途記載しています。

```
Visitas/
├── backend/                 # Goバックエンドサービス（実装済み）
│   ├── cmd/
...
│   ├── DATABASE_REQUIREMENTS.md  # データベース設計
│   ├── SETUP.md                  # セットアップ手順
│   ├── TESTING.md                # テスト方針
│   ├── CICD.md                   # CI/CDパイプライン
...
# Phase 2/3で追加予定のディレクトリ（未実装）
# ├── mobile/          # Flutterモバイルアプリ
# ├── web/             # Web管理画面
# └── backend/internal/ai/  # AI/ML統合
```
